...
ShellRoot {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                let demeter-2.0 = data.trim()

                if (demeter-2.0.length > 0 && demeter-2.0 !== shell.lastWorkspaceTrigger) {
                    let parts = demeter-2.0.split("\t")
                    shell.workspaceTriggerMonitor = parts[4] || ""
                    shell.workspaceOverlayOpen = true
                    workspaceOverlayTimer.restart()
                    shell.closeAllPopups()
                    shell.lastWorkspaceTrigger = demeter-2.0
                }
            }
        }
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (demeter-2.0.length > 0 && demeter-2.0 !== shell.lastLauncherTrigger) {
                    let parts = demeter-2.0.split("\t")
                    shell.openLauncher(parts[1] || "", parts[2] || "apps")
                    shell.lastLauncherTrigger = demeter-2.0
                }
            }
        }
    }
