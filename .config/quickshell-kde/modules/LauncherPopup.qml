...
Rectangle {
    ...
    function activateCurrent() {
        ...
        if (root.mode === "apps") {
            launchProc.command = [
                "sh",
                "-c",
                "/home/streak/.config/quickshell-kde/scripts/app-launch.sh " + root.shellQuote(item.path)
            ]
            ...
            clipCopyProc.command = [
                "sh",
                "-c",
                "/home/streak/.config/quickshell-kde/scripts/clipboard-copy.sh " + root.shellQuote(item.path)
            ]
            ...
            if (expr.length > 0) {
                calcProc.command = [
                    "sh",
                    "-c",
                    "result=$(/home/streak/.config/quickshell-kde/scripts/calc-eval.sh " + root.shellQuote(expr) + ") && printf '%s' \"$result\" | wl-copy"
                ]
                            ...
                            onTextChanged: {
                                ...
                                if (root.mode === "calculator" && text.trim().length > 0) {
                                    calcPreviewProc.command = [
                                        "sh",
                                        "-c",
                                        "/home/streak/.config/quickshell-kde/scripts/calc-preview.sh " + root.shellQuote(text.trim())
                                    ]
    ...
    Process {
        id: clipsProc
        command: ["sh", "-c", "/home/streak/.config/quickshell-kde/scripts/clipboard-list.sh"]
        running: true
    ...
    Process {
        id: calcsProc
        command: ["sh", "-c", "/home/streak/.config/quickshell-kde/scripts/calc-history.sh"]
        running: true
